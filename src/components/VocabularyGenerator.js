...
const VocabularyGenerator = () => {
  ...
  const generateWithDetailedLogging = async () => {
    ...
    try {
      ...
      const aiWords = await geminiService.generateVocabulary(request, existingWords, apiKey);
  ...
  const getStepMessage = () => {
    switch (step) {
      case 'calling': return 'Đang gọi Gemini API...';
      case 'parsing': return 'Đang phân tích phản hồi...';
      case 'adding': return 'Đang thêm từ vào vocabulary...';
    ...
    <div className="max-w-4xl mx-auto p-4 sm:p-6 bg-white rounded-xl shadow-lg border border-gray-100">
      ...
      {result && result.success && (
        <div style={{
          ...
          <strong>✅ Thành công!</strong> Đã thêm {result.wordsAdded} từ vựng mới.
          <div style={{ marginTop: '10px' }}>
            <strong>Các từ mới:</strong>
            <ul style={{ margin: '5px 0', paddingLeft: '20px' }}>
              {result.words.slice(0, 5).map((word, index) => (
                <li key={index}>
                  <strong>{word.english}</strong> - {word.vietnamese} ({word.level})
                </li>
              ))}
              {result.words.length > 5 && (
                <li><em>... và {result.words.length - 5} từ khác</em></li>
              )}
            </ul>
          </div>
        </div>
      ...
      <div style={{ padding: '15px', backgroundColor: '#E0F2FE', borderRadius: '4px' }}>
        <h4 style={{ margin: '0 0 10px 0', color: '#0369A1' }}>💡 Tips for better results:</h4>
        <ul style={{ margin: 0, paddingLeft: '20px', color: '#0369A1', fontSize: '14px' }}>
          <li>Sử dụng yêu cầu cụ thể: "từ vựng về công nghệ AI", "động từ liên quan đến nấu ăn"</li>
