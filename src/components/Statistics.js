...
const Statistics = ({ vocabulary, favorites }) => {
  ...
  const a1Count = vocabulary.filter(w => w.level === 'A1' || (w.level === 'beginner')).length;
  const a2Count = vocabulary.filter(w => w.level === 'A2').length;
  const b1Count = vocabulary.filter(w => w.level === 'B1').length;
  const b2Count = vocabulary.filter(w => w.level === 'B2' || (w.level === 'intermediate')).length;
  const c1Count = vocabulary.filter(w => w.level === 'C1').length;
  const c2Count = vocabulary.filter(w => w.level === 'C2' || (w.level === 'advanced')).length;

  return (
    <div className="bg-white rounded-xl shadow-xl p-6">
      <h3 className="text-xl font-bold text-gray-800 mb-4">Thống kê từ vựng theo CEFR</h3>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="text-center p-4 bg-yellow-50 rounded-lg">
          <div className="text-2xl font-bold text-yellow-600">{favorites.size}</div>
      ...
      <div className="mb-6">
        <h4 className="text-lg font-semibold text-gray-700 mb-3">Phân bố theo cấp độ CEFR</h4>
      ...
      {categories.length > 0 && (
        <div className="mb-6">
          <h4 className="text-lg font-semibold text-gray-700 mb-3">Phân bố theo danh mục từ loại</h4>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
            {categories
              .filter(cat => vocabulary.some(word => word.category === cat.id))
              .map((category) => {
                const count = vocabulary.filter(word => word.category === category.id).length;
                return (
                  <div
                    key={category.id}
                  ...
                  >
                    ...
                    <p className="font-medium text-indigo-800 text-sm">{category.name}</p>
                    <p className="text-lg font-bold text-indigo-600">{count}</p>
                    <p className="text-xs text-indigo-500">{category.description}</p>
                  </div>
                );
              })}
          </div>
      ...
      {vocabulary.length > 0 && (
        <div className="mt-6">
          <h4 className="text-lg font-semibold text-gray-700 mb-3">Từ mới nhất</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {vocabulary
              .sort((a, b) => new Date(b.dateAdded) - new Date(a.dateAdded))
              .slice(0, 6)
              .map((word) => (
                <div key={word.id} className="p-3 bg-gray-50 rounded-lg flex justify-between items-center">
                  <div>
