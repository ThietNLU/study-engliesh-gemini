...
import { BookOpen, Brain, Plus, Sparkles, Settings, BarChart3, Target, Clock, Trophy, Star } from 'lucide-react';
    ...
    <div className="space-y-4 sm:space-y-6 px-3 sm:px-0">
      ...
      <div className="bg-gradient-to-r from-indigo-600 via-blue-600 to-purple-600 rounded-2xl p-4 sm:p-8 text-white">
        <div className="max-w-4xl">
          <h1 className="text-2xl sm:text-4xl font-bold mb-2">Chào mừng trở lại! 👋</h1>
          ...
          {currentWord && (
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3 sm:p-4 mb-4">
              ...
              <p className="text-xl sm:text-2xl font-semibold">{currentWord.english}</p>
              <p className="text-base sm:text-lg opacity-90">{currentWord.vietnamese}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <div className="bg-white rounded-xl p-4 sm:p-6 shadow-lg border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-600 text-xs sm:text-sm font-medium">Tổng từ vựng</p>
              <p className="text-2xl sm:text-3xl font-bold text-blue-600">{totalWords}</p>
            </div>
        ...
        <div className="bg-white rounded-xl p-4 sm:p-6 shadow-lg border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-600 text-xs sm:text-sm font-medium">Yêu thích</p>
              <p className="text-2xl sm:text-3xl font-bold text-yellow-600">{favoriteCount}</p>
            </div>
        ...
        <div className="bg-white rounded-xl p-4 sm:p-6 shadow-lg border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-600 text-xs sm:text-sm font-medium">Độ chính xác</p>
              <p className="text-2xl sm:text-3xl font-bold text-green-600">{accuracyRate}%</p>
            </div>
        ...
        <div className="bg-white rounded-xl p-4 sm:p-6 shadow-lg border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-600 text-xs sm:text-sm font-medium">Điểm quiz</p>
              <p className="text-2xl sm:text-3xl font-bold text-purple-600">{score.correct}/{score.total}</p>
            </div>
      ...
      {/* Quick Actions */}
      <div>
        <h2 className="text-xl sm:text-2xl font-bold text-gray-800 mb-4">Chọn hoạt động</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 border border-gray-100">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg sm:text-xl font-bold text-gray-800">Từ mới nhất</h3>
          ...
          <div className="space-y-3">
            {recentWords.map((word) => (
              <div key={word.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div className="min-w-0 flex-1">
                  <p className="font-semibold text-gray-800 text-sm sm:text-base truncate">{word.english}</p>
                  <p className="text-xs sm:text-sm text-gray-600 truncate">{word.vietnamese}</p>
                </div>
            ...
            {recentWords.length === 0 && (
              <p className="text-gray-500 text-center py-4 text-sm">Chưa có từ vựng nào</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 border border-gray-100">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg sm:text-xl font-bold text-gray-800">Phân bố cấp độ</h3>
              ...
              return (
                <div key={level} className="space-y-1">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-gray-500">{count} từ ({percentage}%)</span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div 
                      className={`h-2 rounded-full ${
                        level === 'A1' ? 'bg-green-400' :
                        level === 'A2' ? 'bg-green-500' :
                        level === 'B1' ? 'bg-yellow-400' :
                        level === 'B2' ? 'bg-orange-400' :
                        level === 'C1' ? 'bg-red-400' :
                        'bg-purple-400'
                      }`}
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
        <div className="flex flex-col md:flex-row items-center justify-between">
          <div>
            <h3 className="text-lg font-bold text-gray-800 mb-1">Sẵn sàng bắt đầu?</h3>
            <p className="text-gray-600">Chọn một hoạt động phía trên hoặc tiếp tục từ nơi bạn đã dừng lại</p>
          </div>
