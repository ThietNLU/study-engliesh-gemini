...
import { Download, Upload, Trash2, Database, Cloud, Smartphone, RefreshCw } from 'lucide-react';
import storageService from '../services/storageService';

const DatabaseManager = ({ onDataCleared }) => {
  const [stats, setStats] = useState({ vocabularyCount: 0, favoritesCount: 0, totalSizeKB: 0 });
  ...
  const loadStats = async () => {
    try {
      ...
      setStats({
        ...
        totalSizeKB: Math.round(JSON.stringify({ vocabulary, favorites: Array.from(favorites) }).length / 1024)
      });
    } catch (error) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
      <div className="flex items-center mb-6">
        ...
        <h2 className="text-2xl font-bold text-gray-800">Quản lý cơ sở dữ liệu</h2>
      </div>

      {/* Storage Status */}
      <div className="mb-6 p-4 bg-gray-50 rounded-lg">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-semibold text-gray-800">Hệ thống lưu trữ</h3>
          <div className="flex items-center">
            ...
            <span className={`px-3 py-1 rounded-full text-sm font-medium ${
              ...
              {currentBackend === 'firestore' ? 'Firestore (Cloud)' : 'Local Storage'}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-blue-50 rounded-lg p-4 text-center">
          ...
          <p className="text-2xl font-bold text-blue-800">{stats.vocabularyCount}</p>
        </div>
        <div className="bg-yellow-50 rounded-lg p-4 text-center">
          <p className="text-yellow-600 text-sm font-medium">Yêu thích</p>
          <p className="text-2xl font-bold text-yellow-800">{stats.favoritesCount}</p>
        </div>
        <div className="bg-green-50 rounded-lg p-4 text-center">
          <p className="text-green-600 text-sm font-medium">Dung lượng</p>
          <p className="text-2xl font-bold text-green-800">{stats.totalSizeKB} KB</p>
        </div>
      </div>
      ...
      <div className="bg-gray-50 rounded-lg p-4">
        <h3 className="font-semibold text-gray-800 mb-2">📍 Thông tin lưu trữ</h3>
        <div className="text-sm text-gray-600 space-y-1">
          <p><strong>Backend:</strong> {currentBackend === 'firestore' ? 'Firebase Firestore' : 'Browser Local Storage'}</p>
          <p><strong>Đồng bộ:</strong> {currentBackend === 'firestore' ? 'Tự động giữa các thiết bị' : 'Chỉ trên thiết bị này'}</p>
          {currentBackend === 'localStorage' && (
            <p><strong>Keys:</strong> englishVocabulary, favorites, geminiApiKey</p>
          )}
        </div>
      </div>
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 max-w-md mx-4">
            <h3 className="text-lg font-bold text-red-600 mb-4">Xác nhận xóa</h3>
