...
const QuizGenerator = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      showNotification(`Đã tạo ${quiz.questions.length} câu hỏi thành công!`, 'success');
    } catch (error) {
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            <div className="mb-4">
              ...
              <div className="space-y-2">
                ...
                >
                  {config.topic ? topics.find(t => t.id === config.topic)?.name || config.topic : 'Chọn chủ đề...'}
                </button>
                      ...
                      <div key={category}>
                        ...
                        {topics.filter(t => t.category === category).map(topic => (
                          <button
                            key={topic.id}
                            onClick={() => {
                              setConfig(prev => ({ ...prev, topic: topic.id }));
                              setCustomTopic('');
                ...
                {savedQuizzes.map(quiz => (
                  ...
                  >
                    ...
                    <div className="text-xs text-gray-500">
                      {quiz.questions.length} câu hỏi • {new Date(quiz.savedAt).toLocaleDateString()}
                    </div>
                  </div>
                ))}
              </div>
          ...
          {generatedQuiz ? (
            <div className="bg-white rounded-xl shadow-lg p-6">
              ...
              <div className="bg-gray-50 rounded-lg p-4 mb-6 grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-green-600">{config.level}</div>
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-orange-600">
                    {generatedQuiz.questions.reduce((sum, q) => sum + (q.points || 1), 0)}
                  </div>
                  ...
                  <div key={question.id || index} className="border border-gray-200 rounded-lg p-4">
                    ...
                    {question.type === 'fill_blank' && (
                      <div className="mt-3">
                        ...
                        {question.acceptable_answers && question.acceptable_answers.length > 1 && (
                          <div className="text-sm text-gray-600">
                            Also acceptable: {question.acceptable_answers.filter(a => a !== question.correct_answer).join(', ')}
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    {question.type === 'sentence_transformation' && (
                      <div className="mt-3 space-y-2">
                        <div className="bg-gray-50 p-2 rounded">
                          <strong>Original:</strong> {question.original_sentence}
                        </div>
                    ...
                    {question.type === 'true_false' && (
                      <div className="mt-3">
                        ...
                        <div className={`font-medium ${question.correct_answer ? 'text-green-600' : 'text-red-600'}`}>
