        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
          ...
          >
            <div className="text-center">
              ...
              <p className="text-gray-600 mb-4">
                Sử dụng AI để tạo bộ câu hỏi theo chủ đề, mức độ và dạng bài tập mong muốn
              </p>
          ...
          >
            <div className="text-center">
              ...
              <p className="text-gray-600 mb-4">
                Xem và thực hiện các quiz đã lưu, theo dõi kết quả học tập của bạn
              </p>
...
const QuickStats = () => {
  ...
  React.useEffect(() => {
    try {
      ...
      const totalQuestions = savedQuizzes.reduce((sum, quiz) => 
        sum + (quiz.questions?.length || 0), 0
      );

      const averageScore = results.length > 0 
        ? results.reduce((sum, r) => sum + r.results.percentage, 0) / results.length
        : 0;
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredQuizzes.map((quiz) => (
          <div key={quiz.id} className="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow">
            ...
            <div className="mb-4 space-y-2">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium">{quiz.config?.level || 'N/A'}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">Dạng:</span>
                <span className="font-medium capitalize">{quiz.config?.type?.replace('_', ' ') || 'N/A'}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">Đã lưu:</span>
                <span className="font-medium">{new Date(quiz.savedAt).toLocaleDateString()}</span>
              </div>
            </div>
      ...
      {filteredQuizzes.length === 0 && (
        <div className="text-center py-8">
          <p className="text-gray-500">Không tìm thấy quiz nào phù hợp với bộ lọc</p>
        </div>
      )}
    </div>
  );
