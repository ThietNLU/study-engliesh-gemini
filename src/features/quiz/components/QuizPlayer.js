...
const QuizPlayer = ({ quiz, onComplete, onExit }) => {
  ...
  const finishQuiz = () => {
    ...
    // Immediately compute and propagate results for persistence/analytics
    try {
      ...
      console.error('Error computing/completing quiz results:', e);
    }
  };
  ...
  const isAnswerCorrect = (question, userAnswer) => {
    ...
    switch (question.type) {
      ...
      case 'fill_blank': {
        const acceptable = question.acceptable_answers || [
      ...
      case 'matching': {
        ...
        return (
          ...
          JSON.stringify(question.correct_matches)
        );
      }
      case 'ordering': {
        return (
        ...
        return question.gaps.every(gap => {
          ...
          return acceptable.some(
            answer =>
              ...
              userGapAnswer?.toLowerCase().trim()
          );
        });
      }
      default:
            ...
            {currentQuestion.type === 'true_false' && (
              <div className='space-y-4'>
                ...
                <div className='flex space-x-4'>
                  <label
                    className={`flex-1 p-4 border rounded-lg cursor-pointer text-center transition-all ${
                      selectedAnswer === true
