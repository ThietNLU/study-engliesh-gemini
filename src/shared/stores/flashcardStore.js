  ...
  switch (rating) {
    ...
    case 'hard':
      ...
      newInterval = Math.max(
        1,
        Math.round(interval * SM2_CONSTANTS.INTERVAL_MULTIPLIER.HARD)
      );
      newRepetitions += 1;
      break;
    ...
    case 'easy':
      ...
      if (newRepetitions === 1) {
        ...
        newInterval = Math.round(
          interval * easeFactor * SM2_CONSTANTS.INTERVAL_MULTIPLIER.EASY
        );
      }
      break;
    ...
    persist(
      (set, get) => ({
          ...
          if (filters.status && filters.status !== 'all') {
            cardsToReview = cardsToReview.filter(card => {
              ...
              if (filters.status === 'learning')
                return card.lastReviewed && card.interval < 21;
        ...
        endStudySession: () => {
          ...
          const duration =
            new Date(endTime) - new Date(state.studySession.startTime);
        ...
        getCardStats: () => {
          ...
          const learning = state.cards.filter(
            card => card.lastReviewed && card.interval < 21
          ).length;
        ...
        getFilteredCards: () => {
          ...
          if (state.filters.category) {
            filtered = filtered.filter(
              card => card.category === state.filters.category
            );
          }
          if (state.filters.level) {
            filtered = filtered.filter(
              card => card.level === state.filters.level
            );
          }
          if (state.filters.status && state.filters.status !== 'all') {
            filtered = filtered.filter(card => {
              if (state.filters.status === 'new') return !card.lastReviewed;
              if (state.filters.status === 'learning')
                return card.lastReviewed && card.interval < 21;
